...
def analyzer(state: NarrAIState) -> dict:
    ...
    if state.get("resume_from") in ("predictor", "writer") or (
        any(c["filename"].startswith("predicted") for c in state["chapters"])
        and os.path.exists(style_analysis_path)
    ):
    ...
    try:
        ...
        save_checkpoint(state["session_dir"], "analyzer")
        if state.get("on_agent"): state["on_agent"]("analyzer", f"error:Analyzer failed — {e}")
